...
class LeafletPolygon
{
    ...
    {   
        $polygonName = $this->name;
        foreach ($this->components as $component) {
            if(is_string($component)){
                $this->codes .= $component;
            } else {
                $this->codes .= $component->result($polygonName);
            }
    ...
    {   
        if(is_string($this->latLng)){
            $this->codes .= "var {$this->name} = L.polygon({$this->latLng}".($this->options?",".json_encode($this->options):"").");\r\n";
        } else {
            $this->codes .= "var {$this->name} = L.polygon(".json_encode($this->latLng).($this->options?",".json_encode($this->options):"").");\r\n";
        }
        if($mapName){
